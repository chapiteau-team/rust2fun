...
fn test_apply() {
    ...
    assert!(map2_product_consistency(Some("str"), None::<usize>, check_length).holds());
    assert!(map2_product_consistency(Some("str"), Some(1), check_length).holds());

    assert!(product_r_consistency(None::<&str>, None::<usize>).holds());
    assert!(product_r_consistency(Some("str"), None::<usize>).holds());
    assert!(product_r_consistency(Some("str"), Some(1)).holds());

    assert!(product_l_consistency(None::<&str>, None::<usize>).holds());
    assert!(product_l_consistency(Some("str"), None::<usize>).holds());
    assert!(product_l_consistency(Some("str"), Some(1)).holds());
}
