...
pub fn invariant_identity<FA>(fa: FA) -> IsEq<FA>
    where FA: Invariant<Target<<FA as Higher>::Param>=FA> + Eq + Clone {
...
pub fn invariant_composition<FA, FB, FC>(
    ...
    where FA: Invariant<Target<FB::Param>=FB> + Invariant<Target<FC::Param>=FC> + Clone,
          FB: Invariant<Target<FC::Param>=FC>,
          FC: Invariant + Eq {
